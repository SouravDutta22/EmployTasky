...
const CompleteTask = ({ data, index, onRemove }) => {
  return (
    <div className='flex-shrink-0 h-full min-w-[260px] sm:min-w-[300px] p-4 sm:p-5 bg-green-950 rounded-xl'>
      ...
      <div className='mt-4 sm:mt-6 flex flex-col gap-2'>
        <div className='w-full bg-green-800 rounded-xl font-medium py-1.5 sm:py-2 px-2 text-xs sm:text-sm text-center'>Completed</div>
        <button 
          onClick={() => onRemove(index)}
          className='w-full bg-red-800 hover:bg-red-700 rounded-xl font-medium py-1.5 sm:py-2 px-2 text-xs sm:text-sm text-center transition-colors'
        >
          Remove
        </button>
      </div>
